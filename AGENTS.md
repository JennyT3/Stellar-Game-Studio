...
3. Implement the required Game Hub client interface:
   ```rust
   ...
   ```
4. Require player auth on `start_game` using `require_auth_for_args` for points.
5. Create/track game state in temporary storage and extend TTL.
6. Call `game_hub.start_game(...)` in `start_game` and `game_hub.end_game(...)` in your game-end path.
7. Add tests in `contracts/<game-name>/src/test.rs` (use the mock Game Hub pattern).
