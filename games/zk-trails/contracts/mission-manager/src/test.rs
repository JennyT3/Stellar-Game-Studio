...
// Unit tests for the mission-manager contract using a simple mock GameHub.
...
use crate::{Error, MissionManagerContract, MissionManagerContractClient};
...
fn setup_test() -> (
    Env,
    MissionManagerContractClient<'static>,
    ...
    // Deploy mission-manager with admin and GameHub address
    let contract_id = env.register(MissionManagerContract, (&admin, &hub_addr));
    let client = MissionManagerContractClient::new(&env, &contract_id);

    // Register mission-manager as a whitelisted game (mock does nothing)
...
fn test_upgrade_function_exists() {
    ...
    // Deploy mission-manager with admin
    let contract_id = env.register(MissionManagerContract, (&admin, &hub_addr));
    let client = MissionManagerContractClient::new(&env, &contract_id);
