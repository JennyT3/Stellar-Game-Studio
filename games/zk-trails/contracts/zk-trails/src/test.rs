...
// Unit tests for the zk-trails contract using a simple mock GameHub.
...
use crate::{Error, ZkTrailsContract, ZkTrailsContractClient};
...
fn setup_test() -> (
    ...
    // Deploy zk-trails with admin and GameHub address
    let contract_id = env.register(ZkTrailsContract, (&admin, &hub_addr));
    let client = ZkTrailsContractClient::new(&env, &contract_id);

    // Register zk-trails as a whitelisted game (mock does nothing)
